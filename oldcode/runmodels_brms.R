...
# load data and scale relevant vars - data1 for full dataset, data2 for sampled dataset for test analyses, data3 for full with min size per country >= 300
datarun <- data7 %>% 
  mutate(age, age.m = scale(age, scale = FALSE)[,1])  %>%
  mutate(AUDIT_SCORE, as.m = scale(AUDIT_SCORE, scale = FALSE)[,1]) %>% 
  filter(ncountry == c("Brazil") |  # select subset of countries to speed up ini tial analyses
           ncountry == c("Colombia") |
           ncountry == c("Denmark") |
           ncountry == c("Finland") |
           ncountry == c("Israel") |
...
saveRDS(brmsout,"GDS_brms_results_data3_negbin.RDS")
