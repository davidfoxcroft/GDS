...
# check that these two lines are in the Makevars file 
...
glimpse(data8)
which(!complete.cases(data8))

datarun <- list(   
   N = dim(data8)[1],
   Y = data8$response,
   K = 2,
   X = matrix(data8$AUDIT_SCORE, data8$message, nrow = N, ncol = K),
   N_1 = length(unique(data8$Final_country)),
   M_1 = 
   J_1 = 
   )
...
make_stancode(bf.drinkless, data8, prior = prior)
...
mod <- readRDS("GDS_brms_data7_form6.RDS")
#mod <- readRDS("fit6.RDS")
...
datarun3 <- datarun2 %>% 
  group_by(message,impact,ncountry) %>% 
...
p2 <- ggplot(data = datarun3, 
             aes(text = paste(ncountry,
                              ...
                              round(hh,2),
                              ")"))) +
   geom_pointrange(aes(x = ncountry, 
               y = med, 
               color = ncountry,
               ymin = ll,
               ymax = hh),
